...
interface SearchBarProps {
    ...
    handleSubmit: (e: FormEvent<HTMLButtonElement>, path: string, value: string) => void;
}

interface RoleSearchListProps {
    path: string;
    handleSubmit: (e: FormEvent<HTMLSelectElement>, path: string, value: string) => void;
}

interface RoleEditListProps {
    uid: string;
    handleRoleUpdate: (e: FormEvent<HTMLButtonElement>, uid: string, role: string) => void;
}
...
export default function ManageUsers() {
    ...
    const [alertStatus, setAlertStatus] = useState<'success' | 'error' | 'info'>('success');
    const [displayAllUsers, setDisplayAllUsers] = useState<boolean>(false);
    const { user } = UserAuth() || {};;

    async function handleSubmit(
        e: FormEvent<HTMLButtonElement | HTMLSelectElement>,
        path: string,
        value: string
    ) {
        e.preventDefault();
        if (value !== '') {
            await fetch(
                process.env.REACT_APP_AIRQUALITY_API_URL + `user/${path}?searchvalue=${value}`,
                {
                    method: 'GET',
                    headers: {
    ...
    async function handleRoleUpdate(
        ...
        e.preventDefault();
        await fetch(
            process.env.REACT_APP_AIRQUALITY_API_URL + `user/${uid}/${role}`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <caption className="p-5 text-left bg-white dark:bg-gray-800">
                    ...
                    <div className="flex justify-between">
                        ...
                        <button
                            className="bg-blue-500 hover:bg-blue-700 text-white font-bold mt-2 py-2 px-4"
                            onClick={() => setDisplayAllUsers(true)}
                        >
                            Display All Users
                        </button>
                    </div>
                ...
                <thead className="text-xs text-gray-600 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th className="items-center justify-start px-4 py-3">
                            User ID
                            <SearchBar
                                path="read-from/uid"
                                searchValue={user_id}
                                setSearchValue={setUser_id}
                        ...
                        <th className="px-4 py-3">
                            Username
                            <SearchBar
                                path="read-from/username"
                                searchValue={username}
                                setSearchValue={setUsername}
                                handleSubmit={handleSubmit}
                            />
                        </th>
                    ...
                    {userinfo && Array.isArray(userinfo) ? (
                        userinfo.map((user) => (
                            <tr key={user.uid}>
                                ...
                                <td className="px-4 py-3 text-xs">{user.username}</td>
                                <td className="px-4 py-3 text-sm">
                                    {user.role}
                                    {editMode ? (
                                        <RoleEditList uid={user.uid} handleRoleUpdate={handleRoleUpdate} />
                                    ) : null}
                                </td>
                            </tr>
                        ))
                        ...
                        <tr>
                            <td className="px-4 py-3 text-sm">
                                No users found. Try searching for a user using the search boxes
                            </td>
    ...
    return (
        <div className="text-gray-600 dark:text-gray-500 focus-within:text-gray-400 dark:focus-within:text-gray-700 flex flex-row ">
            <input
                type="search"
                name="q"
                className="table-filter-input pr-0"
                placeholder="Search..."
                autoComplete="off"
                value={searchValue}
            ...
            <button
                type="submit"
                className="text-black  dark:text-white hover:bg-gray-100 dark:hover:bg-gray-500 p-2.5 rounded-md "
...
const RoleSearchList = ({ path, handleSubmit }: RoleSearchListProps) => {
...
const RoleEditList = ({ uid, handleRoleUpdate }: RoleEditListProps) => {
    const Roles = ['admin', 'sensortech', 'user'];
    ...
    return (
        <div className="text-gray-600 focus-within:text-gray-400 relative flex flex-row items-center">
            ...
            <button
                className="role-save-button focus:shadow-outline"
                onClick={(e) => handleRoleUpdate(e, uid, updatedrole)}
